...
export default function setupCamera(scene, options = {}) {
  ...
  let mouse = { buttons: [false, false, false], x: 0, y: 0, x2: 0, y2: 0 };
	...
	let lockVerticalRotate = options.lockVerticalRotate || false
  ...
  function zoomAlt(factor) {
    while (spinning) {

    }
    if (factor > 0) {
      // tip: get diff between min and max distance/angle
      const newDistance = distance * (1 + factor * zoomFactor)
      const newVerticalAngle = 
      distance = newDistance > 500 ? 500 : newDistance
    } else {
